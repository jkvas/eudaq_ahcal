...
class AhcalBifTluAlpidTableDataCollector:public eudaq::DataCollector{
public:
  AhcalBifTluAlpidTableDataCollector(const std::string &name,
  ...
  static const uint32_t m_id_factory = eudaq::cstr2hash("AhcalBifTluAlpidTableDataCollector");
private:  
  std::mutex m_mtx_map;
  std::deque<eudaq::EventSPC> m_que_cal; //ahcal
  std::deque<eudaq::EventSPC> m_que_bif; //bif
  std::deque<eudaq::EventSPC> m_que_tel; //alpide telescope
  std::deque<eudaq::EventSPC> m_que_tlu; //AIDA TLU
  std::deque<eudaq::EventSPC> m_que_table; //AIDA TLU
  ...
  eudaq::EventSPC m_ev_last_tlu;
  eudaq::EventSPC m_ev_last_table;
  ...
  int16_t m_shift_tgn_tel;
  int16_t m_shift_tgn_tlu;
...
namespace{
  auto dummy0 = eudaq::Factory<eudaq::DataCollector>::
    Register<AhcalBifTluAlpidTableDataCollector, const std::string&, const std::string&>
    (AhcalBifTluAlpidTableDataCollector::m_id_factory);
}

AhcalBifTluAlpidTableDataCollector::AhcalBifTluAlpidTableDataCollector(const std::string &name,
					     const std::string &runcontrol):
...
void AhcalBifTluAlpidTableDataCollector::DoStartRun(){
...
void AhcalBifTluAlpidTableDataCollector::DoStopRun(){
  std::unique_lock<std::mutex> lk(m_mtx_map);
 m_accept_ev = false;
  m_ev_last_cal.reset();
  m_ev_last_bif.reset();
...
void AhcalBifTluAlpidTableDataCollector::DoConfigure(){
  auto conf = GetConfiguration();
  if(conf){
    conf->Print();
    m_shift_tgn_cal = conf->Get("SHIFT_TGN_CAL", int16_t(0));
    m_shift_tgn_tel = conf->Get("SHIFT_TGN_TEL", int16_t(0));
    m_shift_tgn_tlu = conf->Get("SHIFT_TGN_TLU", int16_t(0));
  }
  EUDAQ_INFO("m_shift_tgn_cal "+ std::to_string(m_shift_tgn_cal));
  EUDAQ_INFO("m_shift_tgn_tel "+ std::to_string(m_shift_tgn_tel));
  EUDAQ_INFO("m_shift_tgn_tlu "+ std::to_string(m_shift_tgn_tlu));
}

void AhcalBifTluAlpidTableDataCollector::DoConnect(eudaq::ConnectionSPC idx){
 std::unique_lock<std::mutex> lk(m_mtx_map);
  std::cout<<"connecting "<<idx<<std::endl;
}

void AhcalBifTluAlpidTableDataCollector::DoDisconnect(eudaq::ConnectionSPC idx){
  std::unique_lock<std::mutex> lk(m_mtx_map);
 std::cout<<"disconnecting "<<idx<<std::endl;
}

void AhcalBifTluAlpidTableDataCollector::DoReceive(eudaq::ConnectionSPC idx, eudaq::EventSP ev){
  std::unique_lock<std::mutex> lk(m_mtx_map);
 if(ev->IsFlagFake()){
  ...
  while(!m_que_cal.empty() && !m_que_bif.empty() && !m_que_tel.empty()
    ...
    auto ev_sync =  eudaq::Event::MakeUnique("AhcalBifTluAlpidTable");
    ...
    while(!m_que_tel.empty() && m_que_tel.front()->GetEventN() < tg_n_sync){
      auto ev_sync_only_tel =  eudaq::Event::MakeUnique("AhcalBifTluAlpidTable");
