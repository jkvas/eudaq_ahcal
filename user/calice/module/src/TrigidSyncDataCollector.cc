...
class TrigidSyncDataCollector:public eudaq::DataCollector{
public:
  TrigidSyncDataCollector(const std::string &name,
			  const std::string &rc);
  ...
  static const uint32_t m_id_factory = eudaq::cstr2hash("TrigidSyncDataCollector");
private:
  std::mutex m_mtx_map;
...
namespace{
  auto dummy0 = eudaq::Factory<eudaq::DataCollector>::
    Register<TrigidSyncDataCollector, const std::string&, const std::string&>
    (TrigidSyncDataCollector::m_id_factory);
}

TrigidSyncDataCollector::TrigidSyncDataCollector(const std::string &name,
						 const std::string &rc):
  DataCollector(name, rc){
}

void TrigidSyncDataCollector::DoConnect(eudaq::ConnectionSPC idx){
  std::unique_lock<std::mutex> lk(m_mtx_map);
  m_conn_evque[idx].clear();
  m_conn_inactive.erase(idx);
}

void TrigidSyncDataCollector::DoDisconnect(eudaq::ConnectionSPC idx){
  std::unique_lock<std::mutex> lk(m_mtx_map);
...
void TrigidSyncDataCollector::DoConfigure(){
  m_noprint = 0;
...
void TrigidSyncDataCollector::DoReset(){
  std::unique_lock<std::mutex> lk(m_mtx_map);
  m_noprint = 0;
  m_conn_evque.clear();
  m_conn_inactive.clear();
...
void TrigidSyncDataCollector::DoReceive(eudaq::ConnectionSPC idx, eudaq::EventSP evsp){
  std::unique_lock<std::mutex> lk(m_mtx_map);
  ...
  auto ev_sync = eudaq::Event::MakeUnique("TrigidSync");
